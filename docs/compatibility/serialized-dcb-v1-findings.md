...
| F-006 | On the serialized path, consistency-ness is solely membership in `ConsistencyTags` (`ParseTag`'s second argument); an omitted (unobserved) tag parses as a `FallbackTag`, fails `IsConsistencyTag()`, and is skipped before reservation — no reservation is created, and a null `lastSortableUniqueId` is rejected at the door. On the in-process command path (`CoreGeneralSekibanExecutor.ExecuteAsync`), a consistency tag with a null head is reserved (comparison skipped). | Path-scope clarification, no V1-boundary divergence | Both are true of different paths; the V1 boundary conforms to §3.1 "do not reserve"; the reported concurrent interleaving cannot occur through the five endpoints because the unobserved commit never holds a reservation there. The residue is a mixed-deployment serialization difference between paths — upstream behaviour, not a conformance defect. |
| F-007 | The 2026-09-03 downstream tightening aligns with Sekiban#1174 / SEK-G51: the runtime rejects absent, aliased, mixed, duplicate, case-variant, and explicit-`null` collection members before typed binding. No known repository/client caller intentionally emits an omitted collection. Sekiban.Dcb 10.20.0's presence-only gate permits a present `null` collection to reach null-coalescing, so explicit-`null` handling is a deliberate local divergence. | Accepted shape tightening / versioned-package divergence | Keep the local raw gate at preview.7 even though the dependency baseline is now 10.22.0 (SWR-G087 binder gate unchanged). Fixed local descriptors remain inside the existing `malformed_commit_envelope` message and never contain request content. Explicit empty arrays and all populated official requests retain the existing behavior. The change is request-side only; response golden bytes and JSON shapes are unaffected. |
