...
| TypeScript | Available -- depends on `@sekiban/dcb-client` at exact npm `0.2.0` (published) and `@sekiban/as-wasm` at exact npm `0.1.0` (lane-ready/unpublished). | Not available -- no dev-mode sample exists in the monorepo yet. |
