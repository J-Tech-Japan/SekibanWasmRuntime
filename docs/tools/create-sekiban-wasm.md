...
| Swift | Pre-tag verified -- depends on the repository-root package `github.com/J-Tech-Japan/SekibanWasmRuntime` at exact `1.0.0-preview.4`. Tags `v1.0.0-preview.1`–`.3` predate the root manifest; remote resolution is verified after `v1.0.0-preview.4` is cut. | Not available -- no dev-mode sample exists in the monorepo yet. |
