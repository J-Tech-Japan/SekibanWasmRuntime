...
| npm `@sekiban/dcb-client` | 0.1.0 | npm via `ts-v*` lane (tag `ts-v0.1.0` + `npm-release` env pending, human-gated) | `1.0.0-preview.3` | Lane ready | Extraction smoke: packed-tarball client commit → tag-state → list-query readback (SWR-G057), `npm-ts-preview-readiness.md`; release lane with credential-free dry-run (SWR-G058), `npm-ts-release-lane.md` |
