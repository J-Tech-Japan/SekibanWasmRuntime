...
| Swift `sekiban-swift` (products `SekibanWasm`, `SekibanMv`) | `1.0.0-preview.4` | Repository-root SwiftPM package: `github.com/J-Tech-Japan/SekibanWasmRuntime` at the same bare `v*` release version | `1.0.0-preview.4` | Pre-tag verified | Root manifest build/test and local-package consumer proof (SWR-G074); `v1.0.0-preview.1`–`.3` predate the root manifest and are not SwiftPM-resolvable; remote exact-version proof is a post-tag follow-up |
