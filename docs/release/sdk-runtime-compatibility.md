...
| Swift `sekiban-swift` (products `SekibanWasm`, `SekibanMv`) | 0.1.0 (published, superseded) and 0.1.1 (clean target) | Mirror repo `github.com/J-Tech-Japan/sekiban-swift`; `v0.1.0` remains immutable, `v0.1.1` follows the staged-tree fix and approval gate | `1.0.0-preview.3` | 0.1.1 pending | SWR-G076 redirects SwiftPM scratch output outside the staged tree and asserts the exact published contents; both versions remain documented so publication history is not erased |
