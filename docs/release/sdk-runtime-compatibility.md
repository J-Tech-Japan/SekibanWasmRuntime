...
| Swift `sekiban-swift` (products `SekibanWasm`, `SekibanMv`) | 0.1.1 | Mirror repo `github.com/J-Tech-Japan/sekiban-swift` at clean `v0.1.1` (post-merge tag + approval gate) | `1.0.0-preview.3` | Published target | `v0.1.0` remains published but is superseded because its mirror root contained SwiftPM build output; SWR-G076 redirects scratch output outside the staged tree and asserts the exact published contents |
