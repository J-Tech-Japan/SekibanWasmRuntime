...
The repository-owned TypeScript npm packages `@sekiban/as-wasm`
(`src/lib/sekiban-as-wasm`) and `@sekiban/aspire` (`src/lib/sekiban-aspire-ts`)
release together through the `ts-v*` lane: a published GitHub Release defines
the release moment, the tag defines the package version, and the actual npm
publish is gated behind the protected `npm-release` environment.

`@sekiban/dcb-core`, `@sekiban/dcb-domain`, and `@sekiban/dcb-client` are
**published public npm packages** (currently `0.2.0`). They are not built,
packed, or released from this repository's `ts-v*` lane. Package contents and
metadata for the two repository-owned packages are owned by
`npm-ts-preview-readiness.md` (SWR-G057); this lane only verifies and ships
`@sekiban/as-wasm` and `@sekiban/aspire`.
...
- `scripts/release/check-npm-package-versions.sh <version>` is the gate (it
  ...
  as CI runs it.
- Both repository-owned packages version together; a release always publishes
  both of them.
...
- **verify job (credential-free)** — version gate, `npm ci` + build for both
  repository-owned packages, `npm pack` tarball inventory, the registry-backed
  extraction smoke (`scripts/release/npm-extraction-smoke.sh` — packed
  `@sekiban/as-wasm`, registry `@sekiban/dcb-*` client build/test, and a
  public-container runtime check that self-reports SKIPPED when Docker is
...
- **publish job** — separate job, `environment: npm-release`, runs only for
  ...
  as verify before publishing. Fails fast with an explicit message while
  `NPM_TOKEN` is absent. Publishes `@sekiban/as-wasm` first, then
...
2. Provision npm auth for the `@sekiban` scope (granular automation token or
   ...
   that environment.
3. Confirm both repository-owned package.json files are at the target version
...
5. After publish, verify `npm view @sekiban/as-wasm@X.Y.Z` and
   `npm view @sekiban/aspire@X.Y.Z`, then proceed to the registry-consumer
   proof (SWR-G059). Confirm DCB consumers still pin the published
   `@sekiban/dcb-*@0.2.0` triplet from npm.
...
- The standard remedy is roll-forward: `npm deprecate @sekiban/as-wasm@X.Y.Z
  "<reason, pointer to fixed version>"` (same for `@sekiban/aspire`), bump both
  repository-owned package.json versions, and cut the next `ts-v*` release.
- If one package publishes and the other fails mid-release, fix the cause and
  ...
  duplicate version, so only the missing one actually republishes —
  deprecate-and-bump both if the partial state is user-visible for long.
