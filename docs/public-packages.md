...
**.NET packages** share the repository release line (`1.0.0-preview.N`), because
they are cut from the same GitHub Release. Every **language SDK** carries its
own line starting at `0.1.0`, because each ships on its own lane and moves at
its own pace. So an SDK version never has to match a runtime version — see
...
```xml
<PackageReference Include="Sekiban.Dcb.WasmRuntime" Version="1.0.0-preview.3" />
<PackageReference Include="Sekiban.Dcb.WasmRuntime.Remote" Version="1.0.0-preview.3" />
<PackageReference Include="Sekiban.Dcb.WasmRuntime.Aspire" Version="1.0.0-preview.3" />
```
...
All three sit on Sekiban.Dcb `10.8.0`. `Sekiban.Cloud.Client 1.0.0-preview.1` is
