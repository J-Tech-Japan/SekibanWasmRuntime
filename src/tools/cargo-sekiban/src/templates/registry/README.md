...
`repository-local Sekiban Rust path dependencies` and does not use the unpublished
...
```bash
bash scripts/verify-no-local-sekiban-paths.sh
```
...
`artifacts/samples/__SEKIBAN_PROJECT_KEBAB__/{modules,config}` and are not checked in.
...
RUNTIME_URL=http://localhost:8080 \
  cargo run --manifest-path Cargo.toml \
    --package __SEKIBAN_PROJECT_KEBAB__-client
```
...
It writes a report to `reports/smoke/__SEKIBAN_PROJECT_KEBAB__-smoke.md` and skips
gracefully (exit 0, `Result: SKIP`) when Docker, the .NET SDK, cargo, or the
`wasm32-wasip1` target are unavailable.

### How this differs from `__SEKIBAN_PROJECT_NAME__`

Both samples drive the same public GHCR runtime container, but they prove
different boundaries:

- This sample (`__SEKIBAN_PROJECT_NAME__`) consumes the **published crates.io** Sekiban
  ...
  fails if a local `repository-local Sekiban Rust path dependencies` path or the unpublished
  `sekiban-wasm-domain` helper is reintroduced, and also asserts the AppHost
  targets the public GHCR image.
- `__SEKIBAN_PROJECT_NAME__` proves the public runtime container can execute
