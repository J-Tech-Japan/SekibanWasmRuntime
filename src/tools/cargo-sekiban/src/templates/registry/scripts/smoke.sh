...
SAMPLE_DIR="."
APPHOST="$SAMPLE_DIR/AppHost/__SEKIBAN_PROJECT_PASCAL__.AppHost.csproj"
MANIFEST="$SAMPLE_DIR/Cargo.toml"
ARTIFACT_DIR="$ROOT/artifacts/samples/__SEKIBAN_PROJECT_KEBAB__"
MODULE="$ARTIFACT_DIR/modules/__SEKIBAN_PROJECT_KEBAB__.wasm"
CONFIG="$ARTIFACT_DIR/config/sekiban-manifest.json"
REPORT_DIR="$ROOT/reports/smoke"
REPORT="$REPORT_DIR/__SEKIBAN_PROJECT_KEBAB__-smoke.md"
...
write_report() {
  ...
  mkdir -p "$REPORT_DIR"
  {
    printf '# crates.io Rust Decider Smoke (SWR-G056)\n\n'
    printf '%s\n' "- Result: **$result**"
    printf '%s\n' "- Detail: $detail"
    printf '%s\n' "- Runtime image: \`ghcr.io/j-tech-japan/sekiban-wasm-runtime-host:${SAMPLE_RUNTIME_IMAGE_TAG:-1.0.0-preview.7}\`"
    printf '%s\n' "- Sekiban crates: crates.io \`=0.1.0\` (no local Sekiban path dependencies)"
...
if [[ ! -s "$MODULE" || ! -s "$CONFIG" ]]; then
  log "building Rust WASM module + manifest from crates.io Sekiban crates"
...
log "running typed Rust client smoke (RemoteSekibanExecutor + Command + tag-state + in-memory ListQuery/CountQuery)"
CLIENT_EVIDENCE="$(cargo run --manifest-path "$MANIFEST" --package __SEKIBAN_PROJECT_KEBAB__-client --release 2>>"$APPHOST_LOG")"
client_status=$?
...
for _ in $(seq 1 30); do
  # This sample's WeatherForecast domain has no delete event, so the MV table has
  # no is_deleted column; match on forecast_id only.
