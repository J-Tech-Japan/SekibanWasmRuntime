...
function applyReadmeContentFixes(text, language, mode) {
  if (language === "ts" && mode === "registry") {
    return text
      ...
      .replace(
        "- `tarball` (monorepo only): packs `@sekiban/as-wasm` from `src/lib/sekiban-as-wasm`\n" +
          "  with `npm pack` while the Client installs published `@sekiban/dcb-*@0.1.0`\n" +
          "  from the registry.",
        "- `tarball`: packs `@sekiban/as-wasm` from a monorepo checkout with `npm pack`.\n" +
          "  The Client uses published `@sekiban/dcb-core`, `@sekiban/dcb-domain`, and\n" +
          "  `@sekiban/dcb-client` at exact 0.1.0 from the npm registry.\n" +
