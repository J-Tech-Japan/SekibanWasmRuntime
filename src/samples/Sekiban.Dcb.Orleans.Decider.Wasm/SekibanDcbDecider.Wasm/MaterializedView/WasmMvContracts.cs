...
// MvRowMapper which is not AOT compatible.
//
// The WASM host (Sekiban.Dcb.WasmRuntime.Host) contains a shim
// `WasmBackedMaterializedViewProjector : IMaterializedViewProjector` that bridges these
// WASM-internal types back into Sekiban's MV runtime via host imports / exports.

/// <summary>
/// Kinds of scalar parameter values we support across the WASM boundary. Extending this list
/// requires matching changes in the Host-side shim so Dapper receives a compatible CLR value.
/// </summary>
...
/// token whose shape depends on <see cref="Kind"/>: string for String/Guid/DateTimeOffset/Bytes
/// (base64), number for Int32/Int64/Decimal/Double, bool for Boolean, null for Null.
/// </summary>
public sealed class MvParam
{
    ...
    /// <summary>
    /// Raw JSON token for the value. Null when <see cref="Kind"/> is <see cref="MvParamKind.Null"/>.
    /// </summary>
...
/// Logical names are projector-declared; physical names are registry-resolved per
/// (serviceId × viewName × viewVersion × logicalTable).
/// </summary>
...
/// <summary>
/// Serializable representation of a Sekiban event, constructed by the host from a
/// `SerializableEvent` before calling `mv_apply_event`. Only the fields the projector may
/// inspect are included — payload type name + payload JSON + sortable id + tag strings.
/// </summary>
...
/// <summary>
/// Response envelope returned by `mv_initialize` / `mv_apply_event`. Contains the list of SQL
/// statements the host should execute inside the current apply/init transaction.
/// </summary>
...
/// <summary>
/// A single row returned by an `mv_host_query_*` host import. Columns are JSON tokens so the
/// projector can cast them as needed. See <see cref="MvQueryRowReader"/> for safe accessors.
/// </summary>
...
/// host can run Dapper against the apply-time connection/transaction. Signatures intentionally
/// mirror <c>IMvApplyContext</c> in Sekiban.
/// </summary>
...
/// <summary>
/// WASM-internal projector contract. A WASM MV projector defines its logical tables, emits DDL
/// in Initialize, and transforms one event into zero-or-more SQL statements in ApplyEvent.
/// Projectors that need mid-apply queries use the provided <see cref="IWasmMvQueryPort"/>.
/// </summary>
