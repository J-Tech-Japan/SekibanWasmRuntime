...
/// <summary>
/// Ergonomic builder for SQL parameter lists that need to cross the WASM boundary. Mirrors the
/// shape of anonymous-type parameter objects used by <c>IMaterializedViewProjector</c> on the
/// native side, but with explicit type tagging so the host can rehydrate Dapper parameters
/// without reflection.
/// </summary>
public sealed class MvParamBuilder
{
    ...
    // AOT-friendly JSON string literal producer. Escapes only the characters the JSON spec
    // requires so we don't need to depend on the reflection-based JsonSerializer for scalars.
...
public static class MvQueryRowReader
{
    ...
    public static DateTimeOffset GetDateTimeOffset(MvQueryRowDto row, string columnName) =>
