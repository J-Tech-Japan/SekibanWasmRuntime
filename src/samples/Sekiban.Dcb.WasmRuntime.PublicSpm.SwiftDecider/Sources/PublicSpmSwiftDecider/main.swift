...
// C-ABI entry points for the Swift SPM external-consumer proof module. The
// sample owns its exports (the fixed public products SekibanWasm / SekibanMv
// supply the dispatch helpers, alloc/dealloc, and the MV export helpers);
// projector arrays/factories are rebuilt per call so reactor-mode lazy-init
// quirks never matter.
...
private func primitiveFactories() -> [String: SekibanWasm.PrimitiveHelpers.Factory] {
    [
        WeatherForecastProjection.projectorName: { WeatherForecastProjection() },
        WeatherForecastMultiProjection.projectorName: { WeatherForecastMultiProjection() },
    ]
}
