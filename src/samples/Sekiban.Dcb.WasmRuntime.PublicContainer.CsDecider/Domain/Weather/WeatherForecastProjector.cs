...
// Tag projector — pure state transition (event -> state) for a single forecast.
    ...
    public static ITagStatePayload Project(ITagStatePayload current, Event ev)
    {
        ...
        return ev.Payload switch
        {
            WeatherForecastCreated created => new WeatherForecastState(
                ...
                created.Summary,
                created.CreatedAt),
            WeatherForecastLocationUpdated updated => state with { Location = updated.NewLocation },
            WeatherForecastDeleted deleted => state with { IsDeleted = true, DeletedAt = deleted.DeletedAt },
            _ => state
        };
    }
}
