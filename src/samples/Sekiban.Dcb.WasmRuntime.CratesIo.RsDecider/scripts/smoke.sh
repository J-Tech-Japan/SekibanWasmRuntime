...
APPHOST="$SAMPLE_DIR/AppHost/CratesIoRsDecider.AppHost.csproj"
MANIFEST="$SAMPLE_DIR/Cargo.toml"
ARTIFACT_DIR="$ROOT/artifacts/samples/crates-io-rs-decider"
MODULE="$ARTIFACT_DIR/modules/crates-io-rs-decider.wasm"
CONFIG="$ARTIFACT_DIR/config/sekiban-manifest.json"
REPORT_DIR="$ROOT/reports/smoke"
REPORT="$REPORT_DIR/crates-io-rs-decider-smoke.md"
...
CLIENT_EVIDENCE="$(cargo run --manifest-path "$MANIFEST" --package crates-io-rs-decider-client --release 2>>"$APPHOST_LOG")"
client_status=$?
