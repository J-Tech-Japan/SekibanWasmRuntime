...
public class WeatherApiClient(HttpClient httpClient)
{
    ...
    public async Task<CommandResponse> UpdateLocationAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        // Swift ClientApi's /api/updateweatherforecastlocation endpoint expects
        // { forecastId, newLocation }; the template razor page pipes through a
        // `ChangeLocationName` record whose field name is `NewLocationName` — remap here.
        var payload = new { ForecastId = weatherForecastId, NewLocation = newLocation };
