...
public class WeatherForecastMultiProjectionTests
{
    ...
    [Fact]
    public void Project_ShouldIgnoreUnknownEvent()
    {
        // Given
        var initial = WeatherForecastMultiProjection.GenerateInitialPayload();
        var unknownPayload = new WeatherForecastLocationUpdated(
            "nonexistent", "Nowhere", DateTimeOffset.UtcNow);
        var ev = CreateEvent(unknownPayload, nameof(WeatherForecastLocationUpdated));
