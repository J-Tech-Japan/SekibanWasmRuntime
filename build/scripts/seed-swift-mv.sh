...
# Smoke-test write path for the Swift DCB sample. Hits wasmserver's
# /api/sekiban/serialized/commit directly with three SerializableEventCandidate payloads
# (ClassRoomCreated, StudentCreated, StudentEnrolledInClassRoom) so the MV catch-up worker
# in wasmserver picks them up and the Swift ClientApi's /api/mv/* endpoints start returning
# rows.
#
# The Swift sample intentionally does NOT ship a full command/write API (the issue allows
# scoping the Swift ClientApi to the read surface), so this script is the write-side half of
# the smoke test. Run it once the Aspire AppHost is up.
#
# Usage:
#   ./build/scripts/seed-swift-mv.sh
#   WASM_SERVER=http://127.0.0.1:6299 CLIENT_API=http://127.0.0.1:6298 ./build/scripts/seed-swift-mv.sh

WASM_SERVER="${WASM_SERVER:-http://127.0.0.1:6299}"
CLIENT_API="${CLIENT_API:-http://127.0.0.1:6298}"
...
# Probe: wasmserver should not expose /api/mv/* (generic host).
...
commit_candidate "ClassRoomCreated" \
  "{\"classRoomId\":\"$class_room_id\",\"name\":\"Swift MV Class\",\"maxStudents\":10}" \
  "ClassRoom:$class_room_id"
echo
echo "[seed] committing StudentCreated"
commit_candidate "StudentCreated" \
  "{\"studentId\":\"$student_id\",\"name\":\"Swift MV Student\",\"maxClassCount\":5}" \
  "Student:$student_id"
echo
